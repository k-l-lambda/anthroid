...
import com.anthroid.shared.reflection.ReflectionUtils;
...
import com.anthroid.shared.termux.TermuxConstants.TERMUX_APP;
...
public class TermuxUtils {
     ...
     *
     * If your third-party app is targeting sdk `30` (android `11`), then it needs to add `com.anthroid`
     * package to the `queries` element or request `QUERY_ALL_PACKAGES` permission in its
     * `AndroidManifest.xml`. Otherwise it will get `PackageSetting{...... com.anthroid/......} BLOCKED`
     ...
     * <manifest
     *     <queries>
     *         <package android:name="com.anthroid" />
     *    </queries>
     * </manifest>
     * }
     *
    ...
    public static String getImportantLinksMarkdownString(@NonNull final Context context) {
        ...
        markdownString.append("\n\n### Anthroid Project\n");
        markdownString.append("\n").append(MarkdownUtils.getLinkMarkdownString(TermuxConstants.ANTHROID_APP_NAME, TermuxConstants.ANTHROID_GITHUB_REPO_URL)).append("  ");
        markdownString.append("\n").append(MarkdownUtils.getLinkMarkdownString("Claude Code Documentation", TermuxConstants.CLAUDE_CODE_DOCS_URL)).append("  ");
        markdownString.append("\n").append(MarkdownUtils.getLinkMarkdownString("Anthropic", TermuxConstants.ANTHROPIC_WEBSITE_URL)).append("  ");
        markdownString.append("\n").append(MarkdownUtils.getLinkMarkdownString("API Console", TermuxConstants.ANTHROPIC_CONSOLE_URL)).append("  ");
    ...
    public static String geAPTInfoMarkdownString(@NonNull final Context context) {
        ...
        InputStream inputStream = context.getResources().openRawResource(com.anthroid.shared.R.raw.apt_info_script);
        try {
